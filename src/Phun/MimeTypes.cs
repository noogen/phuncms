namespace Phun
{
    ...
    public static class MimeTypes
    {
        ...
        /// The mime type data
        /// </summary>
        private const string mineTypeData = @"application/andrew-inset ez
        ...
        /// The list
        /// </summary>
        private static readonly Dictionary<string, string> MimeTypeList = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        ...
        static MimeTypes()
        {
            ...
            {
                string[] rows = MimeTypeDataRegEx.Split(mineTypeData);
                foreach (var mimes in rows.Select(item => item.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)))
                {
                    for (var i = 1; i < mimes.Length; i++)
                    {
                        if (!MimeTypeList.ContainsKey(mimes[i]))
                        {
                            MimeTypeList.Add(mimes[i], mimes[0]);
                        }
                    }
                }
            }
        ...
        /// <returns>Content/mime type.</returns>
        public static string GetContentType(string fileExtension)
        {
            fileExtension = (fileExtension + string.Empty).Trim().TrimStart('.');

            // if the file type is not recognized, return "application/octet-stream" so the browser will simply download it
            return List.ContainsKey(fileExtension) ? List[fileExtension] : "application/octet-stream";
        }
    }
}
