namespace Phun.Tests
{
    ...
    public class ContentConnectorTests
    {
        ...
        /// <summary>
        /// Tests the history result in exception for non file.
        /// </summary>
        [TestMethod, ExpectedException(typeof(HttpException))]
        public void TestHistoryResultInExceptionForNonFile()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
            ...
            // Act
            connector.History("/test/", new Uri("http://localhost/test"));
        ...
        /// <summary>
        /// Tests the history result in exception for non existing file.
        /// </summary>
        [TestMethod, ExpectedException(typeof(HttpException))]
        public void TestHistoryResultInExceptionForNonExistingFile()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
        ...
        /// <summary>
        /// Tests the history returns queryable.
        /// </summary>
        [TestMethod]
        public void TestHistoryReturnsQueryable()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
            ...
            fakeUtility.Setup(u => u.Normalize(It.IsAny<string>())).Returns("/test");
            fakeRepository.Setup(r => r.Exists(It.IsAny<ContentModel>())).Returns(true);
            fakeRepository.Setup(r => r.RetrieveHistory(It.IsAny<ContentModel>()))
                          .Returns(new List<ContentModel>().AsQueryable());
            connector.ContentRepository = fakeRepository.Object;
            connector.PathUtility = fakeUtility.Object;

            // Act
            var result = connector.History("/test", new Uri("http://localhost/test"));
        ...
        /// </summary>
        [TestMethod, ExpectedException(typeof(HttpException))]
        public void TestHistoryDataResultInExceptionForNonFile()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
            ...
            // Act
            connector.HistoryData(new ContentModel() { Path = "/test/" }, new Uri("http://localhost/test"));
        ...
        /// </summary>
        [TestMethod, ExpectedException(typeof(HttpException))]
        public void TestHistoryDataResultInExceptionForNonExistingFile()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
        ...
        /// <summary>
        /// Tests the history data result in exception for invalid data string id.
        /// </summary>  
        [TestMethod, ExpectedException(typeof(HttpException))]
        public void TestHistoryDataResultInExceptionForInvalidDataStringId()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
        ...
        /// <summary>
        /// Tests the history data throws exception for invalid result.
        /// </summary>
        [TestMethod, ExpectedException(typeof(HttpException))]
        public void TestHistoryDataThrowsExceptionForInvalidResult()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
            var fakeRepository = new Mock<IContentRepository>();
            var expected = new ContentModel();

            fakeUtility.Setup(u => u.GetTenantHost(It.IsAny<Uri>())).Returns("localhost");
            fakeRepository.Setup(r => r.Exists(It.IsAny<ContentModel>())).Returns(true);
            fakeRepository.Setup(r => r.PopulateHistoryData(It.IsAny<ContentModel>(), It.IsAny<Guid>())).Returns(expected);
            connector.ContentRepository = fakeRepository.Object;
            connector.PathUtility = fakeUtility.Object;

            // Act
            connector.HistoryData(new ContentModel() { Path = "/test", DataId = Guid.NewGuid() }, new Uri("http://localhost/test"));
        }

        /// <summary>
        /// Tests the history data returns valid result.
        /// </summary>
        [TestMethod]
        public void TestHistoryDataReturnsValidResult()
        {
            // Arrange
            var connector = new ContentConnector();
            var fakeUtility = new Mock<ResourcePathUtility>();
            var fakeRepository = new Mock<IContentRepository>();
            var expected = new ContentModel() { DataLength = 100 };

            fakeUtility.Setup(u => u.GetTenantHost(It.IsAny<Uri>())).Returns("localhost");
            fakeRepository.Setup(r => r.Exists(It.IsAny<ContentModel>())).Returns(true);
            fakeRepository.Setup(
                r => r.PopulateHistoryData(
                    It.IsAny<ContentModel>(),
            ...
            // Act
            var result = connector.HistoryData(new ContentModel() { Path = "/test", DataId = Guid.NewGuid() }, new Uri("http://localhost/test"));
